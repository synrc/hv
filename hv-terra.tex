...
resource "proxmox_virtual_environment_vm" "beam_unikernel" {
  ...
  initialization {
    # cloud-init or direct kernel cmdline for seL4
    user_account {
      username = "synrc"
...
LXC-based micro-services are declared with the corresponding container resource and an Alpine ostemplate previously exported by Packer. Kubernetes nodes are declared identically, differing only in the template identifier and subsequent bootstrap provisioners.

Environment-specific values (template IDs, network CIDRs, resource sizes) are supplied via \texttt{tfvars} files under \texttt{environments/}, keeping the modules themselves pure and reusable.
